   ...
   */
	...
	private String composeMapButtonHTML() {
		...
		if (this.numFound > 0) {
			...
			StringBuilder sb = new StringBuilder();
			sb.append(String.format("  <form id=\"mapsearch\" class=\"form-inline\" action=\"%s\" method=\"post\" name=\"mapsearch\">", servlet));
			sb.append("    <input class=\"btn btn-info btn-default\" name=\"submit\" type=\"submit\" value=\"View on Map\" />");
		    sb.append("  </form>\n");
			html = sb.toString();
		}
	...
	private String composeRelevanceHTML() {
		...
		sb.append(String.format("    <input class=\"btn btn-warning\" name=\"submit\" type=\"submit\" value=\"Reset Sort\" %s />", disabled));
	    sb.append("  </form>\n");
		html = sb.toString();
		return html;
	}
	...
	private String composeTableHeaderHTML(boolean showSavedData) {
		...
		if (showSavedData) {
    		...
    		html.append(String.format("            <th width=\"%s\">%s<img alt='Your Data Shelf' src='images/data_shelf.png' title='Your Data Shelf' width='60' height='60'></img>%s</th>\n", dataShelfWidth, dataShelfStartTag, dataShelfEndTag));
		}
